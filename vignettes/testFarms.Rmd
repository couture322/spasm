...
rangesFunc<-function(mvVec,szVec,fsVec){
      ...
      for(k in 1:length(fsVec)){
        ...
        tempDf<-sim_fisheryAq(
          ...
          mutate(size=szVec[j],
                 move=mvVec[i],
                 farmStay=fsVec[k])%>%
          select(size,move,farmStay,ppue)
...
msDf<-rangesFunc(moveVec,sizeVec,farmVec)
head(msDf)
```
...
szFsPlot<-ggplot(fsSz,aes(size,ppue,color=farmStay,group=farmStay))+
  geom_point()+
  geom_line()+
  theme_bw()
