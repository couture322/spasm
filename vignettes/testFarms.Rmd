...
rangesFunc<-function(mvVec,szVec,fsVec,farmHabFact){

  df<-data.frame(size=numeric(),
                 move=numeric(),
                 farmStay=numeric(),
                 habFact=numeric(),
                 ppue=numeric())
  ...
  for(i in 1:length(mvVec)){
          ...
          fleetP <- create_fleet(
            ...
            max_perc_change_f = 0.5,
            effort_allocation = 'simple',
            mpa_reaction = "leave" # changed this from "leave" to "concentrate"
            )
    ...
    for(j in 1:length(szVec)){
      ...
      for(k in 1:length(fsVec)){
        ...
        tempDf<-sim_fisheryAq(
          ...
          select(size,move,farmStay,habFact,ppue)
...
farmVecT<-c(1,10)
habVecT<-c(1,4)
msDfT<-rangesFunc(moveVecT,sizeVecT,farmVecT,habVecT)#test:
head(msDfT)
...
# hours to run # 
#msDf<-rangesFunc(moveVec,sizeVec,farmVec,habVec)#test: rangesFunc(c(0,5),c(0,0.5),c(1,2),c(1,2))
head(msDf)

#write.csv(msDf,file="data/testRunMSFH.csv",row.names = F)
```
