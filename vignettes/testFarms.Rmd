...
rangesFunc<-function(mvVec,szVec,fsVec,farmHabFact){
  ...
  for(i in 1:length(mvVec)){
          ...
          fishP <-
            ...
            larval_movement = 5,
            density_dependence_form = 1 # change to 2 to use the farmStay
            )
...
farmVec<-c(1,seq(10,100,by=10))

habVec<-seq(1,4,by=1)

# hours to run # msDf<-rangesFunc(moveVec,sizeVec,farmVec,habVec)#test: rangesFunc(c(0,5),c(0,0.5),c(1,2),c(1,2))
head(msDf)
