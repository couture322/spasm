...
oaAll<-bind_rows(oaNull,oaMPA,oaAq,oaAqStay)%>%
  group_by(year,run)%>%
...
frmStaySeq<-seq(1,2,by=0.1)

```

Here I am varying the dampening of adult movement at and around the farm (`r frmStay`) and plotting the results with a standard MPA and no MPA as well (from data above). *farmSize* is held constant at 2 patches for this simulation each and *adult_movement* = `r fishPv$adult_movement`.
...
Biomass and catches in MPA and farm scenarios relative to status quo (no farm/MPA) at varying levels of *farmStay*, *farmSize*, and *adult_movement*. 
...
Since the farmStay is included as a muliplier to movement, we get a linear result. *Should* be a linear relationship or other? What else could it be?
...
#     farmStay = as.numeric(attrSz[i,"attraction"]),
#     buffSize=3) %>%
#     group_by(year)%>%
#     summarise(biomass=sum(biomass),
#               catch=sum(numbers_caught))%>%
...
# 
# ggplot(attrSz,aes(attraction,size,color=biomass))+
#   geom_point(size=2)
