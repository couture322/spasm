...
```{r}
frmStay<-c(1,2,5,50,100)
```

Here I am varying the dampening of adult movement at the farm (`r frmStay`) and plotting the results with a standard MPA and no MPA as well (from data above)
...
for(i in 1:length(move)){
  ...
  dfmv <- sim_fisheryAq(
    ...
    select(year, movePar,biomass, catch)

  move<-bind_rows(move,dfmv)  
...
moveDf<-move %>%
  ...
  bind_rows(.,oaAll%>%
              ...
              filter(run %in% c("mpa","noMPA")) %>%
              rename(type=run) %>%
              select(year,movePar, biomass, catch,type))



mvBM<-ggplot(moveDf,aes(year,biomass,color=as.factor(movePar),linetype=type))+
  geom_line()+
  labs()+
  theme_bw()

mvCatch<-ggplot(moveDf,aes(year,catch,color=as.factor(movePar),linetype=type))+
  geom_line()+
  labs()+
  theme_bw()
...
mvBMce<-ggplot(mvCEdf,aes(year,biomass,color=as.factor(movePar),linetype=type))+
  geom_line()+
  labs()+
  theme_bw()

mvCatchCE<-ggplot(mvCEdf,aes(year,catch,color=as.factor(movePar),linetype=type))+
  geom_line()+
  labs()+
  theme_bw()
